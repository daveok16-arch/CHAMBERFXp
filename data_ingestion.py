...
class DataIngestor:
    ...
    def fetch_historical_ohlcv(self, limit: int = 1000, timeframe: str = "5m") -> pd.DataFrame:
        ...
        """
        interval = {"5m": "5m", "1h": "1h", "4h": "4h", "1d": "1d"}.get(timeframe, "5m")
        urls = [
            f"https://api.binance.com/api/v3/klines?symbol={self.symbol_raw}&interval={interval}&limit=1000",
            f"https://api.binance.us/api/v3/klines?symbol={self.symbol_raw}&interval={interval}&limit=1000"
        ]
        for url in urls:
            try:
                candles = []
    ...
    def stream_realtime_data_polling(self, interval_sec: float = 5.0) -> None:
        ...
        """
        urls = [
            f"https://api.binance.com/api/v3/ticker/price?symbol={self.symbol_raw}",
            f"https://api.binance.us/api/v3/ticker/price?symbol={self.symbol_raw}",
        ]
            ...
            while self.is_running_ws:
                for url in urls:
                    try:
                        ...
                        break  # Success - go back to sleep
                    except Exception as e:
                        source = url.split("/")[2]
                        logger.error(f"Error during ticker price poll ({source}): {e}")
                        continue  # Try next mirror before sleeping
