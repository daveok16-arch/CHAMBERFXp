...
// src/utils/rrFramework.ts
function getAssetRRProfile(symbol, date = /* @__PURE__ */ new Date()) {
  ...
  if (cleanSym.includes("JPY")) {
    let fxSession2 = "London Session";
    if (hour >= 0 && hour < 8) fxSession2 = "Asian Session";
    else if (hour >= 16 && hour < 24) fxSession2 = "NY Session";
    let rrRatio = 1.3;
    ...
    return {
      ...
      tpMultiplier: rrRatio,
      slMultiplier: 1,
      sessionName: fxSession2,
      rationale: "BOJ intervention risk, sudden 100-pip reversals. Tighter targets, faster exits.",
      profileSummary: `${basePair} / ${fxSession2} / 1:${rrRatio.toFixed(1)}`,
      badgeStyle: "forex",
      tooltip: `${basePair} ${fxSession2} \u2014 JPY reversal protection profile`
    };
  }
  ...
  if (majors.includes(cleanSym)) {
    ...
    return {
      ...
      profileSummary: `${cleanSym} / ${fxSession2} / 1:1.5`,
      badgeStyle: "forex",
      tooltip: `${cleanSym} ${fxSession2} \u2014 Liquid range profile`
    };
  }
...
function getMarketStatus(symbol, date = /* @__PURE__ */ new Date()) {
  const cleanSym = symbol.toUpperCase().replace(/[^A-Z0-9]/g, "");
  ...
  const hour = date.getUTCHours();
  if (cleanSym.includes("XAU") || cleanSym.includes("GOLD")) {
    const isFridayAfterClose2 = day === 5 && hour >= 22;
    const isSaturday2 = day === 6;
    const isSundayBeforeOpen2 = day === 0 && hour < 23;
    if (isFridayAfterClose2 || isSaturday2 || isSundayBeforeOpen2) {
      return {
        isOpen: false,
        isLowLiquidity: false,
        statusText: "CLOSED",
...
var aiClient = process.env.GEMINI_API_KEY ? new import_genai.GoogleGenAI({
...
async function fetchYahooFinancePrice(symbol) {
  ...
  try {
    ...
    const highRate = ohlc?.high ? Math.max(...ohlc.high.filter((x) => typeof x === "number"), price) : price * 1.002;
    const lowRate = ohlc?.low ? Math.min(...ohlc.low.filter((x) => typeof x === "number"), price) : price * 0.998;
    const changePct = prevClose ? (price - prevClose) / prevClose * 100 : 0;
    const delayed = meta.regularMarketTime ? Date.now() / 1e3 - meta.regularMarketTime > 900 : false;
    return {
      price,
      prevClose,
      highRate,
    ...
    const fallbacks = {
      ...
      "XAUUSD": { price: 4068.5, change: 0.81 },
      "BTCUSD": { price: 67240.5, change: 3.42 },
      "ETHUSD": { price: 3485.2, change: -1.24 },
      "SOLUSD": { price: 142.15, change: 5.61 }
    };
    const key = cleanSymbol.toUpperCase();
    const item = fallbacks[key] || { price: 1, change: 0 };
    const noise = Math.sin(Date.now() / 6e4) * (key.startsWith("BTC") ? 15 : key.startsWith("ETH") ? 1.5 : key.startsWith("SOL") ? 0.15 : key === "XAUUSD" ? 0.35 : key.includes("JPY") ? 0.05 : 7e-5);
...
async function getAssetTechnicalScan(symbol) {
  const cleanSymbol = symbol.endsWith("m") ? symbol.slice(0, -1) : symbol;
  ...
  try {
    if (isCrypto) {
      ...
      if (!cryptoFetched) {
        try {
          ...
          if (responseUs.ok) {
            const rawKlinesUs = await responseUs.json();
      ...
      if (!cryptoFetched) {
        ...
        try {
          ...
          if (resYahoo.ok) {
            const jsonData = await resYahoo.json();
            const result = jsonData?.chart?.result?.[0];
            if (result && result.meta?.regularMarketPrice) {
      ...
      if (cryptoFetched && prices.length > 0) {
        ...
        apiBackoffs.binance.failureCount = 0;
      } else {
      ...
      apiBackoffs.yahoo.failureCount = 0;
    }
    ...
    if (barsCount < 14 || !atr) {
      ...
      if (isGold) atr = 6.5;
      else if (isCrypto) atr = currentPrice * 5e-3;
    ...
    const pivotR1 = Math.round((2 * pivotP - recentLow) * 1e5) / 1e5;
    const pivotS1 = Math.round((2 * pivotP - recentHigh) * 1e5) / 1e5;
    const pivotR2 = Math.round((pivotP + (recentHigh - recentLow)) * 1e5) / 1e5;
    const pivotS2 = Math.round((pivotP - (recentHigh - recentLow)) * 1e5) / 1e5;
    const keyLevelsCalc = {
      s1: recentLow,
      s1Source: "24h Low",
      r1: recentHigh,
      r1Source: "24h High",
      pivotP: Math.round(pivotP * 1e5) / 1e5,
      pivotS1,
      pivotR1,
      pivotS2,
      pivotR2,
    ...
    let confidence = 50;
    let rsiScore = 75;
    if (lastRsi15m > 70) rsiScore = 60;
    else if (lastRsi15m < 30) rsiScore = 60;
    ...
    if (lastRsi15m >= 40 && lastRsi15m <= 60 && macdStatus === maStatus) {
      rawConf = Math.round(normMacd * 0.5 + normEma * 0.5);
    } else {
      rawConf = Math.round(rsiScore * 0.3 + normMacd * 0.35 + normEma * 0.35);
    }
    ...
    const marketStatus = getMarketStatus(symbol, /* @__PURE__ */ new Date());
    if (!marketStatus.isOpen) {
      recommendation = "CLOSED";
      confidence = 0;
    }
    const dir = marketStatus.isOpen && (recommendation.includes("BUY") || recommendation.includes("SELL")) ? recommendation.includes("BUY") ? 1 : -1 : 0;
    let targets = null;
    if (dir !== 0 && marketStatus.isOpen) {
      const rrProfile = getAssetRRProfile(symbol, /* @__PURE__ */ new Date());
      ...
      const stopLoss = dir === 1 ? entry - 1 * atr : entry + 1 * atr;
    ...
    const backoffMs = Math.min(3e5, 15e3 * Math.pow(2, apiBackoffs[apiService].failureCount));
    apiBackoffs[apiService].coolDownUntil = now + backoffMs;
    const errorMsg = String(err?.message || err);
    console.warn(`[API Scan Error - ${apiService}] ${errorMsg}. Applying backoff for ${backoffMs / 1e3}s.`);
    if (memoryScanCache[cleanSymbol]) {
      return {
        ...memoryScanCache[cleanSymbol].data,
        symbol,
    ...
    const price = fallbacks[cleanSymbol.toUpperCase()] || 1.12;
    const fallbackAtr = isGold ? 6.5 : isCrypto ? price * 5e-3 : cleanSymbol.includes("JPY") ? 0.15 : 12e-4;
    ...
    const stopLoss = dir === 1 ? entry - 1 * fallbackAtr : dir === -1 ? entry + 1 * fallbackAtr : price;
    ...
    const fbPivotR1 = Math.round((2 * fbPivotP - fallbackLow) * 1e5) / 1e5;
    const fbPivotS1 = Math.round((2 * fbPivotP - fallbackHigh) * 1e5) / 1e5;
    const fbPivotR2 = Math.round((fbPivotP + (fallbackHigh - fallbackLow)) * 1e5) / 1e5;
    const fbPivotS2 = Math.round((fbPivotP - (fallbackHigh - fallbackLow)) * 1e5) / 1e5;
    return {
      symbol,
      price: price + Math.sin(Date.now() / 45e3) * (isCrypto ? 12 : isGold ? 0.35 : 3e-4),
      ...
      keyLevelsCalc: {
        ...
        r1: fallbackHigh,
        r1Source: "24h High",
        pivotP: Math.round(fbPivotP * 1e5) / 1e5,
        pivotS1: fbPivotS1,
        pivotR1: fbPivotR1,
        pivotS2: fbPivotS2,
        pivotR2: fbPivotR2,
...
async function runGeminiMLScan(symbol, price, changePct, rsi, macd, ema20, ema50, fallbackTargets) {
  const now = Date.now();
  ...
  if (geminiCache[symbol] && now - geminiCache[symbol].timestamp < CACHE_TTL_MS) {
    ...
    const adjustedTargets = {
      entry: Math.round(cachedEntry.targets.entry * ratio * 1e5) / 1e5,
      target1: Math.round(cachedEntry.targets.target1 * ratio * 1e5) / 1e5,
      target2: Math.round(cachedEntry.targets.target2 * ratio * 1e5) / 1e5,
      stopLoss: Math.round(cachedEntry.targets.stopLoss * ratio * 1e5) / 1e5
    };
    return {
      ...cachedEntry,
  ...
  try {
    ...
    if (parsed.recommendation && parsed.targets) {
      ...
      if (dir !== 0 && fallbackTargets && parsed.targets.entry) {
        ...
        const maxDriftPct = isCrypto ? 3e-3 : isGold ? 2e-3 : 5e-4;
    ...
    if (errMsg.includes("429") || errMsg.includes("RESOURCE_EXHAUSTED") || errMsg.includes("quota") || errMsg.includes("limit")) {
      geminiCoolDownUntil = now + 10 * 60 * 1e3;
...
app.get("/api/scan", async (req, res) => {
  ...
  try {
    ...
    if (mlForecast) {
      ...
      const isUnanimous = scanResult.indicatorsScan?.bullishIndicators === 3 || scanResult.indicatorsScan?.bearishIndicators === 3;
      const consensusText = isUnanimous ? "3/3 indicator consensus verified (RSI, MACD, EMA)." : "2/3 indicator consensus (divergence detected \u2014 confidence capped at 75%).";
      res.json({
        ...scanResult,
...
app.get("/api/market-scan", async (req, res) => {
  const assetType = (req.query.type || "").toLowerCase();
  let assets = [
    "BTCUSD",
    "ETHUSD",
    "SOLUSD",
...
app.post("/api/backtest", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      const regime = adx_val >= 23 ? "TRENDING" : "RANGE_BOUND";
      let action = "HOLD";
      ...
      let profit = 0;
      if (position !== 0) {
        const pReturn = position === 1 ? (c.close - entryPrice) / entryPrice : (entryPrice - c.close) / entryPrice;
        const isReversed = position === 1 && action === "SELL" || position === -1 && action === "BUY";
        const shouldExit = isReversed || action === "HOLD" || i === candles.length - 1;
        if (shouldExit) {
          const friction = transactionCost + slippagePct + 2e-4 / 2;
    ...
    const totalReturnPct = (equity - 1e4) / 1e4;
    ...
    if (nTrades > 1) {
      ...
      const downsideVar = downside.length > 0 ? downside.reduce((acc, v) => acc + Math.pow(v, 2), 0) / downside.length : 0;
    ...
    const profitFactor = losses > 0 ? gains / losses : gains > 0 ? 1 : 0;
    res.json({
      metrics: {
        totalReturnPct,
        finalBalance: equity,
